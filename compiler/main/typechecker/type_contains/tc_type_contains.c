...
static bool tc_structtype_contains(struct StructType* expect, struct StructType* actual) {
	...
	if(strcmp(expect->type_name, actual->type_name) != 0) {
		return false;
	}
...
static bool tc_subrtype_contains(struct SubrType* expect, struct SubrType* actual) {
	...
	if(!tc_type_contains(expect->return_type, actual->return_type)) {
		return false;
	}
	if(actual->has_side_effects && !expect->has_side_effects) {
		return false;
	}

	if(expect->count_arg_types != actual->count_arg_types) {
		return false;
	}
...
static bool tc_primitivetype_contains(struct PrimitiveType* expect, struct PrimitiveType* actual) {
	...
	if(expect->is_bool_type && actual->is_bool_type) {
		return true;
	}
	if(expect->is_char_type && actual->is_char_type) {
		return true;
	}
