...
static void annot_private_struct_member(struct ST* st, struct Variable* var) {
	...
	if(!lvst_contains(lvst, var->simple_var->name)) {
		return;
	}
	...
	if(btw->simple_type->struct_type == NULL) {
		return;
	}
	...
	if(has_annotation(memberDecl->super.annotations, ANNOT_PRIVATE)) {
		...
		if(strcmp(line2->_namespace, current_namespace->name) == 0) {
			return;
		}
...
static void annot_private(struct SST* sst, struct Call* call) {
	...
	if(!sst_contains(sst, call->callable->simple_var->name)) {
		return;
	}
