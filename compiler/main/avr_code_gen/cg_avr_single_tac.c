...
void emit_asm_avr_single_tac(struct RAT* rat, struct TAC *tac, struct Ctx* ctx, FILE *fout) {
    ...
    switch(tac->kind){
		...
		case TAC_IF_CMP_GOTO: compile_tac_if_cmp_goto(rat, tac, fout); break;
        ...
        case TAC_LOAD:		compile_tac_load(rat, tac, fout); break;
        case TAC_STORE:		compile_tac_store(rat, tac, fout); break;
    }
}
