...
bool compile_tac_store_local(struct RAT* rat, struct TAC* tac, struct Ctx* ctx, struct IBuffer* ibu) {
	...
	if (offset == 0) {
		avr_stY(reg, "TAC_STORE_LOCAL");
		//st Y, reg; writeback locals
	} else {
		avr_stdY(offset, reg, "TAC_STORE_LOCAL");
	...
	if (wide) {
		...
		avr_stdY(offset + 1, reg + 1, "TAC_STORE_LOCAL");
	} else if (var_width == 2) {
		// in case the value to store is only 8 bit,
		// the upper 8 bit of the variable need
		// to be cleared for subsequent loads.
		avr_ldi(RAT_SCRATCH_REG, 0, "TAC_STORE_LOCAL");
		avr_stdY(offset + 1, RAT_SCRATCH_REG, "TAC_STORE_LOCAL");
	}
