...
bool tac_method(struct TACBuffer* buffer, struct Method* m, struct Ctx* ctx) {
	...
	tacbuffer_append(buffer, makeTACLabelFunction(m->super.line_num, index));

	//setup the stack frame
	if (flags_x86(ctx_flags(ctx))) {
		tacbuffer_append(buffer, makeTACSetupStackframe(m->super.line_num, lvst_stack_frame_size_x86(ctx_tables(ctx)->lvst)));
	} else {
		tacbuffer_append(buffer, makeTACSetupStackframe(m->super.line_num, lvst_stack_frame_size_avr(ctx_tables(ctx)->lvst)));
	}
