...
struct Type* copy_type(struct Type* t) {
	...
	if(t->basic_type != NULL) {
		res->basic_type = copy_basic_type(t->basic_type);
	}
	if(t->type_param != NULL) {
		res->type_param = copy_type_param(t->type_param);
	}
	if(t->array_type != NULL) {
		res->array_type = copy_array_type(t->array_type);
	}
...
struct SimpleType* copy_simple_type(struct SimpleType* s) {
	...
	if(s->primitive_type != NULL) {
		res->primitive_type = copy_primitive_type(s->primitive_type);
	}
	if(s->struct_type != NULL) {
		res->struct_type = copy_struct_type(s->struct_type);
	}
...
struct BasicType* copy_basic_type(struct BasicType* b) {
	...
	if(b->simple_type != NULL) {
		res->simple_type = copy_simple_type(b->simple_type);
	}
	if(b->subr_type != NULL) {
		res->subr_type = copy_subr_type(b->subr_type);
	}
